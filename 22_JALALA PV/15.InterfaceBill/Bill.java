...
public class Bill {
    public static void main(String args[]) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            HashSet<String> productNames = new HashSet<>(); // To track duplicate names

            for (int j = 0; j < m; ) {
                System.out.println("Enter name of product " + (j + 1) + ":");
                String a = sc.next();

                if (productNames.contains(a)) {
                    System.out.println("Error: Duplicate product name detected. Please enter a new product.");
                    continue; // Ask again without incrementing j
                }
                ...
                productNames.add(a); // Add to the set after successful entry
                j++; // Increment only when input is valid
            }
        }
