	...
	public static void main(String[] args) {
		...
		for(int i=0;i<n;i++) {
			...
			for(int j=0;j<m;j++) {
				...
				// Check for duplicate Product ID in current order[i]
				for (int k = 0; k < j; k++) {
				    if (order[i][k].prod_id == b) {
				        System.out.println("Product ID " + b + " already exists in this order. Skipping this product.");
				        isDuplicate = true;
				        break;
				    }
				}
				...
				if (isDuplicate) {
				    j--; // Repeat this iteration (don't count it)
				    continue;
				}
