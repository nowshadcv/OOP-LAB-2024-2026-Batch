...
public class EmployeeObject {
	...
	public EmployeeObject(int no,int sal, String name) {
		eNo=no;
		eSalary=sal;
		eName=name;
	}
	...
	public static void main(String[] args) {
		...
		EmployeeObject employees [ ]=new EmployeeObject [n];
		...
		for(int i=0;i<n;i++) {
			...
			esal=scemp.nextInt();
			EmployeeObject emp=new EmployeeObject(eid,esal,enam);
			employees[i]=emp;
		}
		
		System.out.println("Employee are : \n ");
		for(EmployeeObject y: employees)
			y.showData();
		System.out.println("Enter employee number to search : ");
		int semp=sc.nextInt();
		boolean found=false;
		for ( EmployeeObject e:employees) {
			if(semp==e.eNo) {
				found=true;
